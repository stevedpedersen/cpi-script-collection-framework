...
def logError(Map errorLog, Message message, MessageLog messageLog) {
    ...
    if (errorLog.text) log(Constants.ILCD.ExceptionHandler.MPL_CH_ERR_MSG, errorLog.text, messageLog)
    if (errorLog.statusCode) log(Constants.ILCD.ExceptionHandler.MPL_CH_ERR_STATUS_CODE, errorLog.statusCode, messageLog)
    if (errorLog.exceptionClass) log(Constants.ILCD.ExceptionHandler.MPL_CH_ERR_EXC_CLASS, errorLog.exceptionClass, messageLog)
    if (errorLog.exceptionMessage) log(Constants.ILCD.ExceptionHandler.MPL_CH_ERR_EXC_MSG, errorLog.exceptionMessage, messageLog)
    if (errorLog.errorType) log(Constants.ILCD.ExceptionHandler.MPL_CH_ERR_TYPE, errorLog.errorType, messageLog)
