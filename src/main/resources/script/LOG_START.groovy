...
def logStrict(Message message) {
    return log(message, "START", "INFO", message.getProperty("text"), true)
}
...
def defaultText() { "START of iFlow transaction" }

def Message info(Message message) { 
    return log(message, "START", "INFO", (message.getProperty("text") ?: defaultText())) 
}
def Message debug(Message message) { 
    return log(message, "START", "DEBUG", (message.getProperty("text") ?: defaultText())) 
}
def Message trace(Message message) { 
    return log(message, "START", "TRACE", (message.getProperty("text") ?: defaultText())) 
}
def Message warn(Message message) { 
    return log(message, "START", "WARN", (message.getProperty("text") ?: defaultText())) 
}